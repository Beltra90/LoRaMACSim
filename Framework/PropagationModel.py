...
class RayleighFading:
    ...
    def __init__(self, eta=3, Lambda=300000000 / 868100000, GL=0):
        self.eta = eta
        self.Lambda = Lambda
        self.GL = GL
    ...
    def tp_to_rss(self, indoor: bool, tp_dBm: int, d: float):
        ...
        g_d1 = ((self.Lambda / (4 * np.pi)) ** 2) * (d ** (-self.eta))
        Lpl = 10 * np.log10(g_d1 * np.random.exponential(1)) - bpl
        #Lpl = 10 * np.log10(g_d1 * 0.3) - bpl
        if Lpl > 0:
            Lpl = 0
        #print('Fading (dist = {}, gd1 = {},   bp1 = {}, Lpl = {}, GL = {})'.format(d, g_d1, bpl, Lpl, self.GL))
...
class ETSI:
    ...
    def __init__(self, fc = 868, std=7.8, hb=15, GL = 0):
    ...
    def tp_to_rss(self, indoor: bool, tp_dBm: int, d: float, nfloor=None, innerWalls=None):
        np.random.RandomState(int(d*1000))
        Gn = 1.5
        d1 = np.random.uniform(0, 15)
        alpha = 0.6
        if innerWalls is None:
            innerWalls = int(np.floor(np.random.uniform(0, 4)))
        self.Tor1 = np.sum(np.random.uniform(4, 10, innerWalls))
        self.Tor3 =  d1*alpha
        if nfloor is None:
            nfloor = np.floor(np.random.uniform(0, 5))
        self.GFH =  nfloor*Gn
        bpl = 0  # building path loss
        bpl = np.maximum(self.Tor1, self.Tor3)+np.random.choice([np.random.uniform(4, 11), np.random.uniform(11, 19), np.random.uniform(19, 23)])- self.GFH
        np.random.RandomState()
        Lpl = 40*(1-4/1000*self.hb)*np.log10(d/1000)- 18*np.log10(self.hb)+21*np.log10(self.fc) + np.random.normal(80, self.std) + bpl
        if Lpl <0:
            Lpl = 0
        #print('Fading (dist = {}, Lpl = {},   bp1 = {}, GL = {})'.format(d, Lpl, bpl, self.GL))
        return tp_dBm - self.GL - Lpl
