...
def required_snr(sf):
    ...
    if sf == 7:
        ...
        ValueError('SF {} not supported'.format(sf))
...
class Gateway:
    ...
    def __init__(self, env, location, fast_adr_on=False, max_snr_adr=True, min_snr_adr=False, avg_snr_adr=False,
        ...
        self.pkts_rcvd_all_nodes_online = 0
        self.env = env
    ...
    def packet_received(self, from_node, packet: UplinkMessage, now):
        ...
        if from_node.id not in self.packet_history:
            ...
            self.node_location[from_node.id] = [from_node.location.x, from_node.location.y]
    
        # remove when simulating theoretical
        #if packet.snr < required_snr(packet.lora_param.dr) or packet.rss < self.SENSITIVITY[packet.lora_param.sf]:
        #print('packet.snr = {}'.format(packet.snr))
        #print('packet.rss = {}'.format(packet.rss))
        if packet.snr < required_snr(packet.sf):
            #print('WEAK PACKET (snr = {})'.format(packet.snr))
        ...
        #print(' PACKET Received with (snr = {})'.format(packet.snr))
...
#Code for Scehduling Ack Messages
...
#            schedule_dl = True
    ...
    def get_simulation_data(self, name) -> pd.Series:
        series = pd.Series({
            ...
            'PacketsReceivedAllNodesOnline': self.pkts_rcvd_all_nodes_online,
            'UniquePacketsReceived': self.distinct_packets_received,
            'PacketperNode': self.packet_num_received_from,
            'Location_of_Node': self.node_location,
        })
        series.name = name
